...
<html>
    ...
    <div class="container" > 
        ...
        <p style="text-align: center; font-size: 17px"><b>
            {{-- {{ $active->start_date }} r. - {{ $active->end_date }} r. --}}
            {{ \Carbon\Carbon::createFromFormat('Y-m-d', $active->start_date)->format('d.m.Y') }} r. - {{ \Carbon\Carbon::createFromFormat('Y-m-d', $active->end_date)->format('d.m.Y') }}r.
        </b></p>
        <br/>
        ...
        <table>
            <tr>
                ...
                <td style="font-size: 17px; text-align: justify;">Uniwersytet Rzeszowski kieruje do w/w instytucji studenta(ów) Kolegium Nauk <b>Przyrodniczych</b>, kierunek <b>{{ $active->code->direction->name }}</b> w celu odbycia programowej praktyki zawodowej, według poniższego/dołączonego harmonogramu.</td>
            </tr>
        </table>
        <br/>
